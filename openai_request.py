    ...
    def set(self, ids, payloads):
        ...
        try:
            for i in range(len(ids)):
                ...
                self.cursor.execute(query, (ids[i], payloads[i]))
            self.conn.commit()
        finally:
            self._unlock_table()
...
class AltGenerator:
    ...
    def generate(self, images, ids):
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a helpful assistant designed to output a single class or description.",
            }
        ]
        ...
        for i in range(3):
            try:
                ...
                return [content.strip() for _ in images]
    ...
    def __call__(self, batch):
        ...
        if new_images:
            ...
            self.db.set(new_img_ids, generation)
            cached_alt_texts.update(dict(zip(new_img_ids, generation)))

        batch[self.args.generated_alt_text_column] = [
            cached_alt_texts[img_id] for img_id in img_ids
        ]
        return batch
